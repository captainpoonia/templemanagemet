...
if(isset($_SERVER["REQUEST_METHOD"]) && $_SERVER["REQUEST_METHOD"] == "POST"){
	...
	if(empty($temp)){
		$temple_name_err = "Please enter user name.";
	}else{
		$temple_name = $temp;
	}
	...
	if(empty($temple_name_err) && empty($password_err)){
		...
		$sql = "SELECT admin_name,admin_pass FROM admin WHERE admin_name = ?";
				...
				if(mysqli_stmt_num_rows($stmt) == 1){
					...
					if(mysqli_stmt_fetch($stmt)){
						if(strcmp($password,$h_password)==0){
							...
							$_SESSION["user_name"] = $temple_name;
                             $_SESSION["user"] ="admin";
							/*Redirect to welcome page*/
							header("location: http://localhost/web_project/temple/admin/admin_con.php");
						}else{
							$password_err = "The password you entered was not valid.";
						}
					}
				}else{
					$temple_name_err = "user name does not exist.";
				}
			}else{
